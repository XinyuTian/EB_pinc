...
for (i in beg:end){
	...
	smooth_e <- 10/(mean(c(length(G1),length(G2)))/(res_expr*2)) # rules of thumb
	smooth_pr <- trunc(10/(mean(c(length(G1),length(G2)))*length(IDs_promoter)/(res_pr*res_expr))) # rules of thumb
	smooth_gb <- trunc(10/(mean(c(length(G1),length(G2)))*length(IDs_body)/(res_gb*res_expr))) # rules of thumb
	###########################################################
	...
	for (j in 1:length(temp)) {
		...
		X <- seq(round(max(lambda-(4*sqrt(lambda)),0)),round(lambda+(4*sqrt(lambda))))
